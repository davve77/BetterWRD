    ...
    */
    ...
    function previewPost(postTitle, postContent){
        ...
        */
        ...
        // Show preview
        newMainDiv.innerHTML = `<div class="theme2 round border1 padding"> <p> ${mainDiv.firstElementChild.firstElementChild.outerHTML} </p> <h1 style="display: flex;justify-content: space-between;"> <span style="font-weight: 400; font-size: 25px;">${(postTitle == null) ? 'New Reply' : (postTitle.trim() == '') ? 'No title' : postTitle}</span> </h1> </div> <br> <div style="text-align: right"> <a onclick="stopPreview(document.querySelector('.previewPost'), document.querySelector('main'))" class="theme1 round border1 btn_newrelpy" style=" cursor: pointer; padding: 12px; ">Stop Previewing</a> </div> <div class="theme1 replygroup round border1"> <div class="theme2 thread_replierdata"> <a><div id="prPfp" style="cursor: pointer; background-image: url(https://cdn.wearedevs.net/images/avatars/anovatar.png);" class="thread_pfp"></div></a> <div class="userdesc" style="flex: 1"> <a style="cursor: pointer;"><p id="prName">You</p></a> <p class="usertitle" id="prAlias"></p> <a class="theme1 border1 btnmention" style=" border-radius: 10px; cursor: pointer; ">Mention</a> </div> <div class="userstats"> <p>Posts: 5</p> <p>Threads: 5</p> <p id="prJoined">Joined: Jun, 2019</p> <p>Reputation: <span class="good" id="prRep">6</span></p> </div> </div> <div class="replycard"> <div style="position: relative"> <p class="thread_replytime padding">Posted ${TimeStamp.Beautify(new Date())}</p> </div> <div class="thread_replycontent"><p>${postContent}</p></div> </div> <div class="signature theme2 padding" style="max-height: none;"><p style="">&nbsp;</p> <p style="text-align: center; outline: none;" contenteditable="true" spellcheck="false" id="editableSiggy">Click here to edit your epic siggy.</p> <p style="">&nbsp;</p></div> </div> <br> <div style="text-align: right"> <a onclick="stopPreview(document.querySelector('.previewPost'), document.querySelector('main'))" class="theme1 round border1 btn_newrelpy" style=" cursor: pointer; padding: 12px; ">Stop Previewing</a> </div>`
        newMainDiv.querySelector('.btnmention').style.display = (location.href.endsWith('newthread')) ? 'none' : 'inline-block'

        // Editable siggy
        let siggy = newMainDiv.querySelector('#editableSiggy')
        let savedSiggy = localStorage['bwrd_previewsiggy']
        let range = document.createRange()
        let selection = window.getSelection()

        if(savedSiggy) siggy.textContent = savedSiggy

        siggy.addEventListener('click', ()=> {
            range.selectNodeContents(siggy)
            selection.removeAllRanges()
            selection.addRange(range)
        })
        siggy.addEventListener('input', ()=> {
            localStorage['bwrd_previewsiggy'] = siggy.textContent
        })
