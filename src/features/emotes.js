...
if(document.querySelector('#editor')){
    ...
    */
    ...
    const emotesHTML = `
    <div class="theme1 border1 round" id="emotesdiv" style="position: absolute; bottom: 0; left: -240px; height: 320px; width: 230px; display: flex; flex-direction: row; overflow: hidden; transition: .08s opacity cubic-bezier(0.4, 0, 1, 1);">
    <div id="emotesView" style="position: absolute; width: 100%; height: 100%; flex: 1; display: flex; flex-direction: column;"> <div class="padding"> <div id="emoteSearchDiv" class="theme2" style=" position: relative; display: flex; flex-direction: row; font-size: 14px; border-radius: 8px; height: 30px; "> <input id="emoteSearch" type="text" placeholder="Emotes" style="outline: none;background: 0!important;height: 100%;width: 100%;text-indent: 8px;font-size: 14px;border: 0!important;"> <div style="display: flex;align-items: center;padding: 0 8px;flex: 1;"><svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 0 24 24" width="16px" fill="currentColor" style=" transform: rotateY(-180deg); opacity: .7; "><path d="M0 0h24v24H0V0z" fill="none"></path><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg></div> <div class="underline" style=" display: block; block-size: 100%; inline-size: 100%; position: absolute; border-bottom: 1px solid #ffffff45; border-radius: 5px; overflow: hidden; pointer-events: none; "></div></div></div>
    <div class="scrollemotes" style=" flex: 1; overflow: overlay; ">
    ...
    <div class="theme2" id="createEmote" style="display: none; flex-direction: column; justify-content: center; padding: 10px; padding-bottom: 5px; gap: 10px; ">
    ...
    .emoteContainer{transition: .07s background; border-radius: 6px;} .backbtn:hover{background: #ffffff0f;} .sectionexpand:hover{opacity: 1!important;} .scrollemotes, .twitchscrollemotes{overflow: hidden!important;} .scrollemotes:hover, .twitchscrollemotes:hover{overflow: overlay!important;}`
    ...
    */
    ...
    function refreshEmotes(){
        ...
        JSON.parse(localStorage.getItem('bwrd_emotes')).forEach(emote => {
            ...
            emoteHTML = `<div title="${emote.name}" class="emoteContainer" style=" overflow: hidden; min-height: 36px; max-height: 36px; min-width: 36px; max-width: 36px; cursor: pointer; display: flex; align-items: center; justify-content: center;"><img class="emoteimg" src="${emote.url}" style="max-height: 30px; max-width: 30px;"> </div>`
    ...
    */
    ...
    document.body.addEventListener('click', (e)=> {
        if(e.target.className[0] != undefined && e.target.classList && e.target.classList.length != 0){
            ...
            tinymce.activeEditor.execCommand('mceInsertContent', false, `<p><img class="emoteBWRD" src="${e.target.src}" style="max-width: 60px; max-height: 60px;"><p style="display: none;">${e.target.src}</p></img></p>`)
        }
    })
    ...
    */
    ...
    /*
    *
    * // Create Show/Hide Emotes Button //
    *
    */
    ...
    function showHideEmotes(isonload){
        saved = localStorage.getItem('bwrd_emotestate')
        emotediv = document.querySelector('#emotesdiv')
