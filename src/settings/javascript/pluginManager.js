...
chrome.storage.local.get(['plugins'], (saved)=>{
    ...
    async function refreshPluginList(plugins, list, isFirstLoad){
        ...
        plugins.forEach(plg => {
            ...
            plgDiv.innerHTML = `<div plgName="${plg.name}" class="plgCard" style="height: 170.5px; transition: .15s cubic-bezier(0.3, 0, 0.5, 1); width: 400px; background: rgb(40,40,40);border: 1px solid rgba(255,255,255,0.04);box-shadow: rgba(0, 0, 0, 0.1) 0px 10px 50px;display: flex;flex-direction: column;border-radius: 8px;overflow: hidden;text-align: left!important;"> <div style="height: 60px;background: rgb(35,35,35);padding: 15px;display: flex;align-items: center;justify-content: space-between;"> <div style="display: flex;flex-direction: row;"> <div class="material-icons-outlined" style="display: flex;align-items: center;padding-right: 10px;"><span>extension</span></div> <div> <h1 style="font-size: 17px;margin: 0!important;color: rgb(242,242,242); white-space: pre; max-width: 250px; overflow: hidden; text-overflow: ellipsis;">${util.strip(plg.name)}</h1> <p style="margin: 0;font-size: 13px;opacity: .6; white-space: pre; max-width: 250px; overflow: hidden; text-overflow: ellipsis;">by ${util.strip(plg.author)}&nbsp; —&nbsp; v${util.strip(plg.version)}</p> </div> </div> <div><label class="switch"> <input plgName="${plg.name}" id="pluginswitch" type="checkbox" autocomplete="off"> <span class="slider round"></span> </label></div> </div> <div style="padding: 15px;"> <p style="font-size: 15px; white-space: pre; text-overflow: ellipsis; overflow: hidden;">${util.strip(plg.description)}</p> </div> <div style="padding: 10px;flex: 1;display: flex;justify-content: space-between;"> <div style="display: flex;align-items: center;padding-left: 5px;gap: 7px;opacity: .7;"><a href="https://github.com${new URL(plg.source).pathname.replace('/main/', '/blob/main/')}" style="border-radius: 6ps;" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" fill="white" width="22" height="22" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"></path></svg></a></div> <div style="display: flex;gap: 7px;"><button class="btn btn-primary" id="updatePlg" plgName="${plg.name}" type="button">Update</button><button class="btn btn-primary" id="deletePlg" plgName="${plg.name}" type="button">Delete</button></div></div></div>`
    ...
    function loadPlugin(PLGDATA, ExistingPlugins){
        ...
        let stringData = JSON.stringify(PLGDATA)

        // Return if plugin data is null
        if(!PLGDATA || stringData.match(/<script|<style/gm) || !lib.errorCheck(PLGDATA)) {showToast('Failed to load plugin.'); return}
        ...
        for(var i=0;i<ExistingPlugins.length;i++) {if(ExistingPlugins[i].name == PLGDATA.name){
            lib.overwritePlugins([PLGDATA], ExistingPlugins)
    ...
    async function loadPluginURL(PLURL, ExistingPlugins){
        ...
        const PlgData       = lib.getPluginData(PluginText)
    ...
    async function updatePlugin(plugin, allplugins){
        ...
        // Errors
        if(!getPlgSrc || !getPlgVer || !lib.getPluginData(getPlgSrc)) {showToast('Failed to update this plugin.'); return}
        if(plugin.version == getPlgVer) {showToast(`"${plugin.name}" is up to date.`); return}
        if(lib.getPluginData(getPlgSrc).name != plugin.name) {showToast('Plugin names don\'t match.'); return}
        ...
        // Toast
        showToast(`"${plugin.name}" has been updated.`)
    }
    ...
    dropDiv.addEventListener('drop', (e)=> {
        ...
        reader.onload = (e) => {
            chrome.storage.local.get('plugins', (sv)=> {
                loadPlugin(lib.getPluginData(e.target.result), JSON.parse(sv.plugins))
            })
        }
