...
class PlgLib {
    ...
    getPluginData(plgCode){
        try{
            var NAME          = plgCode.match(/(?<=@name )(.*)/gm)[0]
            var VERSION       = plgCode.match(/(?<=@version )(.*)/gm)[0]
            var DESCRIPTION   = plgCode.match(/(?<=@description )(.*)/gm)[0] || 'No description'
            var AUTHOR        = plgCode.match(/(?<=@author )(.*)/gm)[0] || 'No author'
            var SOURCE        = plgCode.match(/(?<=@source )(.*)/gm)[0]
        }
        catch {return null}
        return {'name': NAME, 'version': VERSION, 'description': DESCRIPTION, 'author': AUTHOR, 'source': SOURCE, 'code': plgCode}
    }
    ...
    setPluginInfo(script, plgObject){
        script.setAttribute('name', plgObject.name)
        script.setAttribute('version', plgObject.version)
        script.setAttribute('source', plgObject.source)
    }
    ...
    loadPlugins(plugins){
        ...
        plugins.forEach(plg => {
            ...
            // Plugin Info
            this.setPluginInfo(pluginScript, plg)
        ...
        // Error handler
        if(!this.errorHandlerExists){
            this.errorHandlerExists = true
    ...
    /*
    async localServerPlugin(){
        const plgURL = 'http://127.0.0.1:63614/debugPlugin.bwrd.js'
        await fetch(plgURL).then(e => {

            let plgCode = e.text()
            let plgData = this.getPluginData(plgCode)
            let plgScript = document.createElement('script')

            if(!plgData) return

            plgScript.textContent = plgCode
            this.setPluginInfo(plgScript, plgData)
            document.head.appendChild(plgScript)
        })
    }
    */
