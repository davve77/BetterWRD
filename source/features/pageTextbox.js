...
if(window.location.href.includes('page')) {input.placeholder = window.location.href.split('=')[1].split('&')[0]}
...
input.addEventListener('change', ()=>{
    if(!isNaN(parseInt(input.value))){ // Checks if input.value is a number
        if(window.location.href.includes('page')) {window.location = `${window.location.href.split('=')[0]}=${input.value}`}
        else {window.location.href += `?page=${input.value}`}
    }
})
